...
public class UserController {
    ...
    public JSONData register(@RequestBody RegisterInfo user) {
     ...
     * @return JSON相关信息
     */
    @RequestMapping("/increaseUserPoint")
    public JSONData increaseUserPoint(int userId, double point) {
        int result = userService.increaseUserPoint(userId, point);
        if (result == 1) {
            return JSONData.buildSuccess();
        } else {
            return JSONData.buildError(29, "用户点数增加出错");
        }
    }
